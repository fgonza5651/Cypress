...
const btnFunerariaParvuloDestacado = '#cont-swipper-otras-opciones > swiper > div > div.swiper-slide.ng-star-inserted.swiper-slide-active > div > div.cont-img-producto'
...
class CremacionParvuloMobile {
    ...
    //Agrega al carrito el articulo Funeraria parvulo y revisa que la suma del total sea correcta
    //Ademas revisa si la casilla quede marcada con un check
    ...
    //Agrega al carrito el articulo Funeraria parvulo destacado y revisa que la suma del total sea correcta
    //Ademas revisa si la casilla quede marcada con un check
    agregarFunerariaParvuloDestacado(){
        cy.get(btnDetalleFuneraria,{timeout:100000}).should('be.visible').click()
        cy.get(btnFunerariaParvuloDestacado,{timeout:100000}).click()
        cy.get(precioAgregado,{timeout:100000}).invoke('text').then((text) => {
            cy.log(text);
            cy.wrap(text).as('precioFuneraria')
        })
...
const cremacionParvuloMobile = new CremacionParvuloMobile ()
export default cremacionParvuloMobile